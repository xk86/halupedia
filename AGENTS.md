...
- Entry point: [src/server/index.ts](REPO_ROOT/src/server/index.ts)
- Config loading: [src/server/config.ts](REPO_ROOT/src/server/config.ts)
- Persistence and schema bootstrapping: [src/server/db.ts](REPO_ROOT/src/server/db.ts)
- LLM transport: [src/server/llm.ts](REPO_ROOT/src/server/llm.ts)
- Prompt lookup and template rendering: [src/server/prompts.ts](REPO_ROOT/src/server/prompts.ts)
- Retrieval and chunk indexing: [src/server/retrieval.ts](REPO_ROOT/src/server/retrieval.ts)
...
5. Render prompt templates from `config/prompts.toml`.
...
- SPA entry: [src/client/App.tsx](REPO_ROOT/src/client/App.tsx)
...
- If a feature needs new prompt text, add it to `config/prompts.toml`, wire it through typed config, and cover it with tests.
...
- Prefer adding or updating tests in [tests/site-smoke.test.ts](REPO_ROOT/tests/site-smoke.test.ts) and [tests/article-regressions.test.ts](REPO_ROOT/tests/article-regressions.test.ts), or add a new focused test file if the concern is distinct.
