...
export function PromptModelsPane({ associations, savingKey, onUpdate }: Props) {
  return (
    <Pane id="prompt-models" title="Prompt Models" count={`${associations.length} prompts`} wide>
      {associations.length ? (
                ...
                <tr key={item.key}>
                  ...
                  <td>
                    <select
                      ...
                      onChange={(e) => onUpdate(item.key, e.target.value as "heavy" | "light", item.thinking)}
                    >
                  ...
                  <td>
                    <label className="admin-thinking-toggle">
                      <input
                        ...
                        disabled={savingKey !== null}
                        onChange={(e) => onUpdate(item.key, item.model, e.target.checked)}
                      />
