...
- `reference_recursive_max_per_article`: traversal fan-out safety limit — max candidates pulled from a single traversed article per depth step, applied independently to each graph direction (forward sidecar refs AND backward backlinks — see `listWrittenBacklinks` in `src/server/db.ts`).
- `reference_recursive_article_limit`: hard cap on the *total* recursively-discovered articles admitted to the final reference list (both directions combined), applied after ranking all recursive candidates by score (descending, stable slug tie-break). Independent of `reference_cull_top_k`, which culls the whole assembled pool (RAG + recursive + reranked priors) together. Entries discovered via a forward reference get `source: "recursive"`; entries discovered because another admitted article links back to the seed get `source: "backlink"` — same scoring/caps, distinct only for log/debug provenance.
