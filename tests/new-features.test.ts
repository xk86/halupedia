...
test("ensureDykHasSourceLink: fact with halu link → converts to wiki link", () => {
  // Halu links in DYK are wrong — the function should replace them with wiki-path links
  ...
  assert.match(result, /\/wiki\/Glow_Fruit/, "should replace halu link with wiki path");
...
test("ensureDykHasSourceLink: fact mentions title as plain text → first occurrence becomes wiki link", () => {
  const fact = "... Glow Fruit was discovered in the southern craters.";
  const result = ensureDykHasSourceLink(fact, "glow-fruit", "Glow Fruit");
  assert.match(result, /\[Glow Fruit\]\(\/wiki\/Glow_Fruit\)/, "should linkify with wiki path");
...
test("ensureDykHasSourceLink: fact does not mention title → link appended/prepended", () => {
  const fact = "... the craters glow at night due to bioluminescent fungi.";
  const result = ensureDykHasSourceLink(fact, "glow-fruit", "Glow Fruit");
  // A wiki-path link must appear; no halu link
  assert.match(result, /\/wiki\/Glow_Fruit/, "should insert wiki source link when title absent");
...
test("ensureDykHasSourceLink: case-insensitive title match", () => {
  ...
  const result = ensureDykHasSourceLink(fact, "glow-fruit", "Glow Fruit");
  assert.match(result, /\/wiki\/Glow_Fruit/);
...
test("DYK facts in homepage cache always contain a wiki link to source article", async (t) => {
  const { root, databasePath } = createTestDb();
  t.after(() => rmSync(root, { recursive: true, force: true }));

  // ensureDykHasSourceLink should insert a /wiki/ path link, NOT a halu link
  ...
  // Must contain a wiki path link, not a halu link
  assert.match(fixed, /\/wiki\//, "fixed fact should contain a /wiki/ path link");
  assert.doesNotMatch(fixed, /halu:/, "DYK facts must NOT use halu links");
});

/* ─────────────────────────────────────────────────────────────────
   5. DYK: already-has-wiki-link is unchanged
   ───────────────────────────────────────────────────────────────── */

test("ensureDykHasSourceLink: fact already has wiki link to source → unchanged", () => {
  // Inline wiki-path links use standard markdown [Label](/wiki/Page)
  const fact = "... [Glow Fruit](/wiki/Glow_Fruit) was discovered in the craters.";
  const result = ensureDykHasSourceLink(fact, "glow-fruit", "Glow Fruit");
  assert.equal(result, fact, "fact with existing wiki link should not be modified");
});

test("ensureDykHasSourceLink: title in plain text → replaced with wiki link", () => {
  const fact = "... Glow Fruit was discovered in the southern craters.";
  const result = ensureDykHasSourceLink(fact, "glow-fruit", "Glow Fruit");
  assert.match(result, /\[Glow Fruit\]\(\/wiki\/Glow_Fruit\)/, "should linkify first occurrence with wiki path");
  assert.doesNotMatch(result, /halu:/, "must not insert a halu link");
